...
export default function VenuePage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      try {
        ...
        // Venue doesn't exist yet, load futsal owner data for auto-fill
        if (venueError.response?.status === 404) {
          console.log('No existing venue, loading futsal owner data for auto-fill...');
        }
      }
      ...
      try {
        ...
        if (ownerResponse.data) {
          ...
          showToast.info('Form pre-filled with your registration data');
        }
      } catch (ownerError) {
  ...
  const populateVenueData = (venue) => {
    ...
    // Media
    if (venue.media) {
      setGalleryImages(venue.media.images || []);
      setGalleryVideos(venue.media.videos || []);
    }
  ...
  const validateForm = () => {
    ...
    if (galleryImages.length === 0) {
      errors.push('At least one Venue Image');
    }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      // Transform frontend state to match backend schema
      const payload = {
