...
import { getBookingById, initiateSplitPayment, verifySplitPayment, payShare, cancelBookingByLeader } from '../store/bookingStore';
...
export default function SplitPaymentPage() {
  ...
  const isLeader = booking && (booking.leaderId?.toString?.() === currentUserId || booking.user?.toString?.() === currentUserId || booking.user?._id?.toString?.() === currentUserId);
  ...
  const handlePayKhalti = async () => {
    try {
      ...
      showToast.error(e.response?.data?.message || 'Failed to initiate payment');
    } finally {
      setPaying(false);
    }
  };
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <div className={`flex-1 p-6 ${isSidebarCollapsed ? 'ml-20' : 'ml-64'}`}>
          <div className="max-w-md mx-auto text-center py-12">
            <p className="text-gray-600 mb-4">No booking specified. Use the link shared by your team leader to pay your share.</p>
            <button onClick={() => navigate('/player/bookings')} className="text-emerald-600 font-medium">Go to my bookings</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex min-h-screen bg-gray-50">
        ...
        <div className={`flex-1 p-6 ${isSidebarCollapsed ? 'ml-20' : 'ml-64'}`}>
          ...
          <button onClick={() => navigate('/player/bookings')} className="mt-4 text-emerald-600">Back to bookings</button>
        </div>
      </div>
    );
  }
      ...
      <div className={`flex-1 p-6 ${isSidebarCollapsed ? 'ml-20' : 'ml-64'}`} style={{ width: `calc(100% - ${isSidebarCollapsed ? '5rem' : '16rem'})` }}>
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6 flex items-center justify-between">
            ...
            <span className="text-lg font-bold text-gray-900">Rs. {booking.pricing?.totalAmount}</span>
          </div>
          ...
          {booking.bookingStatus === 'confirmed' && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <button onClick={() => navigate(`/player/bookings/${bookingId}`)} className="text-emerald-600 text-sm mt-1">View booking</button>
              </div>
            </div>
          )}
          ...
          {/* Leader: list all players and status */}
          {isLeader && booking.bookingStatus === 'pending' && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
              ...
              <ul className="space-y-3">
                {(booking.splitPlayers || []).map((p) => {
                  const uid = p.userId?._id?.toString?.() || p.userId?.toString?.();
                  const name = p.userId?.name || 'Player';
                  return (
                    <li key={uid} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      <span className="font-medium">{name}</span>
                      <span className="text-gray-600">Rs. {p.amountAssigned}</span>
                      {p.paymentStatus === 'paid' ? (
                        <span className="text-emerald-600 text-sm flex items-center gap-1"><CheckCircle className="w-4 h-4" /> Paid</span>
                      ) : (
          ...
          {!isLeader && myShare && booking.bookingStatus === 'pending' && !expired && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
              ...
              {myShare.paymentStatus === 'paid' ? (
                <p className="text-emerald-600 flex items-center gap-2"><CheckCircle className="w-5 h-5" /> You have paid</p>
              ) : (
          ...
          {expired && booking.bookingStatus === 'pending' && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <p className="text-amber-800">Payment deadline passed. This booking may be cancelled automatically.</p>
            </div>
          )}
        </div>
      </div>
